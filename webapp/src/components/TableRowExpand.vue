<template>
  <tbody class="text-sm">
    ...
    <tr>
      ...
      <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
        <div class="flex items-center text-gray-800">
          <div class="w-10 h-10 flex-shrink-0 flex items-center justify-center bg-gray-100 rounded-full mr-2 sm:mr-3">
    ...
    <tr :id="`description-${order.id}`" role="region" :class="!descriptionOpen && 'hidden'">
      <td colSpan="10" class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
        <div class="flex items-center bg-gray-50 p-3 -mt-3">
          <svg class="w-4 h-4 flex-shrink-0 fill-current text-gray-400 mr-2">
  ...
  setup(props, context) {
    ...
    const typeIcon = (type) => {
      switch (type) {
        case 'Subscription':
          return (
            `<svg class="w-4 h-4 fill-current text-gray-400 flex-shrink-0 mr-2" viewBox="0 0 16 16">
